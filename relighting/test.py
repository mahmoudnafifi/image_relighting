...
if __name__ == "__main__":
    ...
    if task == 'normalization':
        ...
        for fn in imgfiles:
            ...
            # get image size
            in_img = in_img.resize((maxSize, maxSize), 3)
        ...
        if input_dir_t is None:
            ...
            if args.ensemble is True:
                if os.path.exists(os.path.join(args.model_dir, 'relighting_net_one_to_any__.pth')):
                    # load net
                    net_patches = Relighting.Relighting(task='relighting_one_to_any')
                    logging.info(
                        "Loading model {}".format(os.path.join(args.model_dir, 'relighting_net_one_to_any__.pth')))
                    net_patches.load_state_dict(
                        torch.load(os.path.join(args.model_dir, 'relighting_net_one_to_any__.pth'), map_location=device))
                    net_patches.eval()
                    net_patches.norm_net.to(device=device)
                    net_patches.relighting_net.to(device=device)
                    net_patches.task = 'relighting_one_to_any'
                else:
                    raise Exception('Relighting Model not found!')
            ...
            with torch.no_grad():
                for fn in imgfiles:
                    ...
                    if args.ensemble is True:
                        ...
                        result_p = net_patches(tensor_in_img, t=tensor_t_img)
                        result_p = torch.squeeze(result_p).cpu().detach().numpy().transpose((1, 2, 0))
                        result_p = result_p[:d1, :d2, :]
                        
                        result = result/2 + result_p/2
